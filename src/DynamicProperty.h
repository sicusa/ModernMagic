...
public:
	...
	const T &Get() const
	{
		if (_info->Permission == PropertyPermission::Writeonly) {
			throw PropertyReadFailed("Writeonly property");
		}
		if (_info->Hash != typeid(T).hash_code()) {
			throw PropertyCaseFailed("Type error");
		}
	...
	void Set(const T &value)
	{
		if (_info->Permission == PropertyPermission::Readonly) {
			throw PropertyWriteFailed("Readonly property");
		}
		if (_info->Hash != typeid(T).hash_code()) {
			throw PropertyCaseFailed("Type error");
		}
...
public:
	...
	DyFunctorProperty(const PropertyInfo *info, const Setter &setter, const Getter &getter);
...
public:
	...
	typedef std::unordered_map<String, std::unique_ptr<PropertyInfo>> PropertyInfoContainer;
