...
private:
	virtual bool OnInstalling() override
	{
		...
		if (!rootobj) {
			MM_THROW InstallingFailed("ChangeProperty: Root Timeline 没有绑定到任何对象");
			return false;
		}

		_property = rootobj->GetProperty(_proname);
		if (!_property) {
			MM_THROW InstallingFailed("ChangeProperty: '" + _proname + "' 属性没有找到");
			return false;
		}
		if (typeid(T).hash_code() != _property->GetHash())  {
			MM_THROW InstallingFailed(
...
protected:
	virtual bool OnInstalling() override
	{
		...
		auto rootobj = this->GetRootObject();
		if (!rootobj) {
			MM_THROW InstallingFailed("PropertyAnimation: Root Timeline 没有绑定到任何对象");
			return false;
		}

		_property = rootobj->GetProperty(_proname);
		if (!_property) {
			MM_THROW InstallingFailed("PropertyAnimation: '" + _proname + "' 属性没有找到");
			return false;
		}
		if (typeid(T).hash_code() != _property->GetHash())  {
			MM_THROW InstallingFailed(
